...
public class Game {
    ...
    /**
     * Constructs a game with a given list of players. Useful to test and give a specific composition of players to the game.
     *
    ...
    public boolean gameOver() {     // Currently, the game is over as soon as a player reaches a score of 9 or the tilesDeck is empty
        ...
        for (Player p : players) {
            ...
            if (p.getScore() >= 9 && !emperorDistributed) {
                ...
                p.giveEmperor();
                TakenokoServer.print(String.format("Player #%d has won the emperor !", p.getId()));
                this.hasEmperor = p;
            }
    ...
    public void start() {           // Starts the game: while the game isn't over, each player plays
        ...
        while (!gameOver()) {
            ...
            if (playerNumber == 0) {   // If it's the first player turn, I.E. we are at the beginning of a turn
                ...
                TakenokoServer.print("Beginning of turn number " + ++turnNumber);    // We print the new turn number
            }
    ...
    private void printRanking() {
        ...
        for (Player pl : players) {
            TakenokoServer.print("Player #" + pl.getId() + " has " + pl.getScore() + " points (" + pl.getClass().getSimpleName() + ")");
        }
    }
